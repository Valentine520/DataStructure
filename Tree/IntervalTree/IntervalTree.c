...
RBTreePointer Insert(RBTreePointer Tree, Item data) {
    ...
    if (!Tree) {
        ...
        New_node->P = Pre;
        New_node->max=data.high;
        //在右侧插入
        //以low（区间起点）作为关键字进行BST的插入删除
        if (data.low> Pre->data.low)
            Pre->Rchild = New_node;
            //在左侧插入
        else
            Pre->Lchild = New_node;
        RBTreePointer In_node=New_node->P;
        //第一阶段从下往上更新max节点的值
        //如果为根节点仍然是可以更新的
        while(In_node!=Reserve->P){
            In_node->max= MAX(In_node->Lchild->max,In_node->Rchild->max,In_node->data.high);
            In_node=In_node->P;
        }
        Tree= RB_Insert_Fixup(Reserve,New_node);//只需调整旋转即可
...
RBTreePointer LeftRotate(RBTreePointer Tree,RBTreePointer Node){
    RBTreePointer nil = Tree->P;
    //Node为原来的节点 被旋转
    RBTreePointer temp=Node->Rchild;//新的根节点
    ...
    Node->P=temp;
    temp->max=Node->max;
    Node->max=MAX(Node->Rchild->max,Node->Lchild->max,Node->data.high);
...
void InorderTraverse(RBTreePointer Tree,RBTreePointer nil) {
    ...
    if (Tree == nil ) {
        ...
        printf("Low=%d High=%d Max=%d\n", Tree->data.low,Tree->data.high,Tree->max);
...
bool RBDelete(RBTreePointer *Tree,Item data){
    ...
    //直接从x向上调整即可
    RBTreePointer adj=x->P;
    while(adj!=(*Tree)->P){
        adj->max=MAX(adj->Rchild->max,adj->Lchild->max,adj->data.high);
        adj=adj->P;
    }
...
RBTreePointer Search(RBTreePointer Tree, Item data) {
    ...
    while (Temp != Tree->P) {
        if (data.low == Temp->data.low) {
            return Temp;
        } else if (Temp->data.low < data.low) {
            Temp = Temp->Rchild;
        } else {
            Temp = Temp->Lchild;
        }
...
RBTreePointer Interval_Search(RBTreePointer Tree,Item i){
    RBTreePointer x=Tree;
    RBTreePointer Nil=Tree->P;
    while(x!=Nil&& !In_Interval(x->data,i)){
        //说明左边必有包含i的区间
        if(x->Lchild!=Nil&&x->Lchild->max>=i.low){
            x=x->Lchild;
        }
        else{
            x=x->Rchild;
    ...
    if(x==Nil){
        printf("The interval is not exist\n");
    }
    return x;//如果为Nil则表示该区间未查找到
}
