...
static int  my_compare(Item data1,Item data2);//比较元素大小
...
void my_union(S_map *L1,const S_map *L2){
    ...
    for(int i=0;i<L2->length;i++){
        //如果遇到不在L1中的元素则插入 插入则是以一种使全部元素移动的方式插入
...
//L3很有讲究  按照compare指定的方式进行归并 (基于值的拷贝)
void my_merge(const S_map* L1,const S_map *L2,S_map *L3,
              int (*my_compare)(Item data1,Item data2)){
    if(ListEmpty(*L1)&& ListEmpty(*L2)){
        L3=NULL;
...
//比较函数
static int my_compare(Item data1,Item data2){
    int flag;
